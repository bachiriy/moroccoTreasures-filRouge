...
<aside class="ml-[-100%] fixed z-10 top-0 pb-3 px-6 w-full flex flex-col justify-between h-screen border-r border-red-900 bg-gradient-to-r from-gray-950 to-red-950 transition duration-300 md:w-4/12 lg:ml-0 lg:w-[25%] xl:w-[20%] 2xl:w-[15%]">
    <div>
        ...
        <ul class="space-y-2 tracking-wide mt-8">
            ...
            <li>
                <a href="/dashboard/products" class="relative px-4 py-3 flex items-center space-x-4 rounded-xl text-white bg-gradient-to-r from-red-800 to-cyan-800">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
...
<div class="ml-auto lg:w-[75%] xl:w-[80%] 2xl:w-[85%] overflow-hidden bg-gradient-to-l from-gray-900 to-red-950 h-screen">
    <div class="sticky z-10 top-0 lg:py-2.5 bg-gradient-to-l border-b border-red-900 from-gray-900 to-red-950">
        <div class="px-6 flex items-center justify-between space-x-4 2xl:container">
            <h5 hidden class="text-2xl text-gray-300 font-medium lg:block">Products</h5>
                            ...
                            <tr class=" hover:bg-gray-500/15">
                                ...
                                <td class="text-sm text-gray-300 font-light px-6 py-4 whitespace-nowrap">
                                    <div class="relative group flex items-center">
                                        ...
                                        <span class="absolute top-full left-0 w-64 rounded-lg bg-gradient-to-l from-gray-900 to-green-950 text-white px-4 py-2 text-xs opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                            <span class="underline">email</span> : {{ $product->user->email }}
                                        </span>
                                    </div>
                                </td>
                                <td class="text-sm text-gray-300 font-light px-6 py-4 ">
                                    <p class="relative group">
                                        {{ $product->name }}
                                        <span class="absolute top-full left-0 w-64 whitespace-pre-line rounded-lg bg-gradient-to-l from-gray-900 to-green-950 text-white px-4 py-2 text-xs opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-50">
                                            <span class="underline"> description </span> : {{ $product->description }} <br>
                                            <span class="underline"> media </span> :
                                            <span class="flex">
                                                @foreach($product->media as $item)
                                                <img src="{{ asset('storage/'. $item->name) }}" class="h-12 mx-1 my-3 flex rounded-sm">
                                                @endforeach
                                            </span>
                                        </span>
                                    </p>
                                ...
                                <td class="text-sm text-gray-300 font-light px-6 py-4">
                                    <p class="relative group">
                                        ...
                                        <span class="absolute top-full left-0 w-64 whitespace-pre-line rounded-lg bg-gradient-to-l from-gray-900 to-green-950 z-50 text-white px-4 py-2 text-xs opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                            <span class="underline">description </span> : {{ $product->category->description }}
                                        </span>
                                    </p>
                                </td>
                                <td class="text-sm text-gray-300 font-light px-6 py-4 whitespace-nowrap">
                                    <a class="text-blue-600 hover:underline" href="/products/{{ $product->id }}">
                                        <p>View</p>
                                    </a>
                                </td>
                                <td class="text-sm text-gray-300 flex justify-center font-light px-6 py-4 whitespace-nowrap">
    ...
    <a href="/products/create">
        <button class="absolute bottom-10 right-8 hover:opacity-70 border border-green-600 bg-gradient-to-l from-green-500  to-green-950 text-gray-200 p-2 rounded-full shadow-lg">Create Product</button>
    </a>
    <a href="/products">
        <button class="absolute bottom-28 right-8 hover:opacity-70 border border-green-600 bg-gradient-to-l from-blue-500  to-blue-950 text-gray-200 p-2 rounded-full shadow-lg">See My Products</button>
    </a>
</div>
