...
<aside class="ml-[-100%] fixed z-10 top-0 pb-3 px-6 w-full flex flex-col justify-between h-screen border-r border-red-900 bg-gradient-to-r from-gray-950 to-red-950 transition duration-300 md:w-4/12 lg:ml-0 lg:w-[25%] xl:w-[20%] 2xl:w-[15%]">
    <div>
        ...
        <ul class="space-y-2 tracking-wide mt-8">
            ...
            <li>
                <a href="/dashboard/requests" class="relative px-4 py-3 flex items-center space-x-4 rounded-xl text-white bg-gradient-to-r from-red-800 to-cyan-800">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
...
<div class="ml-auto lg:w-[75%] xl:w-[80%] 2xl:w-[85%] overflow-hidden bg-gradient-to-l from-gray-900 to-red-950 h-screen">
    <div class="sticky z-10 top-0 lg:py-2.5 bg-gradient-to-l border-b border-red-900 from-gray-900 to-red-950">
        <div class="px-6 flex items-center justify-between space-x-4 2xl:container">
            <h5 hidden class="text-2xl text-gray-300 font-medium lg:block">Requests</h5>
    ...
    <div class="flex flex-col">
        <div class="overflow-x-auto sm:mx-0.5 lg:mx-0.5">
            <div class="py-2 inline-block min-w-full sm:px-6 lg:px-8">
                            ...
                            <tr class=" hover:bg-gray-500/15">
                                ...
                                <td class="text-sm text-gray-300 flex justify-center font-light px-6 py-4 whitespace-nowrap">
                                    <form action="/dashboard/requests/{{$request->user->id}}/approve" method="post" class="w-fit">
                                        @csrf
                                        <button class="text-green-500 mx-2 hover:underline" onclick="return confirm('Are you sure you want to approve seller request ?')">Approve Seller</button>
                                    </form>
                                    <form action="/dashboard/requests/{{$request->user->id}}/deny" method="post" class="w-fit">
                                        @csrf
                                        <button class="text-red-500 mx-2 hover:underline" onclick="return confirm('Are you sure you want to deny seller request ?')">Deny Seller</button>
                                    </form>
                                </td>
                            </tr>
                            @endforeach
                ...
                @if(count($requests) === 0)
                <p class="m-10 text-gray-300">No seller requests were made.</p>
                @endif
            </div>
        </div>
    </div>
