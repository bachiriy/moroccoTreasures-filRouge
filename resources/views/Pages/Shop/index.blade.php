...
@section('content')
    ...
    <!-- Tab Menu -->
    <div
        class="flex flex-wrap items-center duration-100 py-10 justify-center transition-all bg-white text-gray-800">
        <p class="catBtns border-2 border-gray-200 p-4 rounded-lg mx-6 cursor-pointer" onclick="window.location.href = '/shop'">All</p>
        @foreach(\App\Models\Category::all() as $category)
            <button value="{{ $category->name }}" rel="noopener noreferrer"  onclick="filterByCategory(this, {{ $category->id }})" class="catBtns cursor-pointer flex items-center flex-shrink-0 px-5 py-3 space-x-2text-gray-600">
    ...
    <script>
        ...
        let clearBtns = () => {
            catBtns.forEach((btn) => {
                btn.classList.remove('border-2', 'border-gray-200', 'p-4', 'rounded-lg');
            });
        }
        ...
        let filterByCategory = (self, category) => {
            section.innerHTML = '';
            clearBtns();
            self.classList.add('border-2', 'border-gray-200', 'p-4', 'rounded-lg');
            ...
            fetch(`http://127.0.0.1:8000/api/shop/${category}`, {
                ...
                .then(data => {
                    if (data.products.length === 0) {
                        section.innerHTML = '<p class="text-center text-xl my-8 font-bold">No records found.</p>'
                    } else {
                            ...
                            section.innerHTML += `
                                 <article
                                    ...
                                    <a href="/products/${product.id}">
                                        ...
                                        <div class="mt-3 p-2">
                                            <h2 class="text-lg font-semibold text-slate-700">${product.name}</h2>
                                            <p class="text-sm text-slate-400">${self.value}</p>
